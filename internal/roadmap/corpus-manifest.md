...
| `metadata.pdf` | `C:\Users\garae\Documents\pypdf\resources\metadata.pdf` | pass after metadata parser exists | Document info dictionary: title, author, subject, keywords, dates. |
| `missing_info.pdf` | `C:\Users\garae\Documents\pypdf\resources\missing_info.pdf` | pass after metadata parser exists | Missing `/Info` is not an error; metadata should be empty. |
...
Next concrete step: wire hard v0.1 pass/refusal tests for `hello-world.pdf`,
`two-different-pages.pdf`, and `encrypted-file.pdf`, then add metadata fixtures
once document-info parsing exists.
